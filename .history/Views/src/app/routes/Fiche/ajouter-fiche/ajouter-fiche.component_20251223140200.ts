...
export class AjouterFicheComponent implements OnInit {
  ...
  validateForm(): boolean {
    ...
    if (!this.snapForm.get('dateFin')?.value) {
      missingFields.push('Date de fin');
    }
    ...
    if (dateVisibilite > dateFin) {
      this.toastSrv.error(
        'La date de visibilité ne peut pas être postérieure à la date de fin',
        'Erreur de validation'
      );
      return false;
    }
    ...
    if (dateReception > dateDebut) {
      this.toastSrv.error(
        'La date de réception ne peut pas être postérieure à la date de début',
        'Erreur de validation'
      );
      return false;
    }
  ...
  handleOnSumit() {
    ...
    if (!this.validateForm()) {
      this.toastSrv.error("Merci de remplir tous les champs obligatoires", 'Erreur', {
          timeOut: 5000,
          progressBar: true,
        });
    }else{
this.isSubmitting = true;
    ...
    this.userService.addFiche(formdata).subscribe({
      next: message => {
        this.toastSrv.success("Vous venez d'ajouter un nouveau document", 'Succès', {
          timeOut: 3000,
          progressBar: true,
        });
        this.isSubmitting = false;
      ...
      error: error => {
        this.toastSrv.error("Une erreur s'est produite lors de l'ajout du document", 'Erreur', {
          timeOut: 5000,
          progressBar: true,
        });
        console.log(error);
