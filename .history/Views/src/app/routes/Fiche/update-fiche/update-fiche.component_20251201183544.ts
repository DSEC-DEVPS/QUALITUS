  ...
  ngOnInit() {
    ...
    this.userService.getAllCategorieWithTheirSousCategorie().subscribe({
      next: result => {
        ...
        console.log(this.ListecategorieSousCategorie);
        console.log('---');
      },
      error: error => {
        console.log(error);
    ...
    this.userService.getOneFiche(id).subscribe({
      next: resultat => {
        ...
        this.snapForm
          ...
          ?.setValue(
            this.ListecategorieSousCategorie.filter(
              item => item.id === resultat.id_Categorie
            )[0].Sous_Categorie.filter(sub => sub.id === resultat.id_SousCategorie)[0].id
          );
