...
export class TokenInterceptor implements HttpInterceptor {
  ...
  intercept(req: HttpRequest<unknown>, next: HttpHandler) {
    ...
    if (this.tokenService.valid() && this.shouldAppendToken(req.url)) {
      const totken = this.tokenService.getBearerToken();
      console.log('le token est', totken);
      console.log(req.headers.append('Authorization', this.tokenService.getBearerToken()));
      return next
        .handle(
          req.clone({
