...
import { AjouterGrilleComponent } from '../ajouter-grille/ajouter-grille.component';
import * as XLSX from 'xlsx';
import { UpdateGrilleComponent } from '../update-grille/update-grille.component';
import {
  AppearanceAnimation,
...
@Component({
  ...
  templateUrl: './liste-grille.component.html',
  styleUrl: './liste-grille.component.scss',
})
export class ListeGrilleComponent implements OnInit {
  private readonly toastSrv = inject(ToastrService);
  dataSource!: MatTableDataSource<grille>;
  msg = '';
  destroy$: Subject<boolean> = new Subject<boolean>();
  displayedColums: string[] = ['id', 'nom', 'Etat', 'dateCreation', 'Actions'];
  @ViewChild(MatPaginator) paginator!: MatPaginator;
  filename = `Liste_Grille${new Date()}.xlsx`;
  downloadData!: grille[];
  ...
  openDialog_update(id: number): void {
    this.dialog.open(UpdateGrilleComponent, {
      data: { id: id },
      height: 'calc(100% - 30px)',
      width: 'calc(100% - 30px)',
      maxWidth: '100%',
  ...
  delete(id: number) {
    ...
    newConfirmBox
      ...
      .pipe(
        tap(value => {
          if (value.success) {
            this.userService.deleteGrille(id).subscribe({
              next: resultat => {
