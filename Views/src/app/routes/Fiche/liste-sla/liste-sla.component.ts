...
import { AjouterSlaComponent } from '../ajouter-sla/ajouter-sla.component';
import * as XLSX from 'xlsx';
import { UpdateSlaComponent } from '../update-sla/update-sla.component';
import {
  AppearanceAnimation,
...
@Component({
  ...
  templateUrl: './liste-sla.component.html',
  styleUrl: './liste-sla.component.scss',
})
export class ListeSlaComponent implements OnInit {
  ...
  filename = `Liste_SLA${new Date()}.xlsx`;
  downloadData!: sla[];
  constructor(private userService: UserService) {}
  ngOnInit(): void {
    this.userService.getAllSla().subscribe(user => {
  ...
  openDialog(): void {
    this.dialog.open(AjouterSlaComponent, {
  ...
  openDialog_update(id: number): void {
    this.dialog.open(UpdateSlaComponent, {
      data: { id: id },
      height: 'calc(100% - 30px)',
      width: 'calc(100% - 30px)',
      maxWidth: '100%',
  ...
  delete(id: number) {
    ...
    newConfirmBox
      ...
      .pipe(
        tap(value => {
          if (value.success) {
            this.userService.deleteSla(id).subscribe({
              next: resultat => {
